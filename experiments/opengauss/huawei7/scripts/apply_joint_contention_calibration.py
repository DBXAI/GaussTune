#!/usr/bin/env python3
"""Apply an exact-config joint contention correction to frozen recommendations."""
...
from huawei7.joint_contention import validate_joint_contention_evidence
...
def main() -> int:
    ...
    if (
        ...
        or Path(str(base_ref.get("path", ""))).resolve() != args.recommendations.resolve()
        or base_ref.get("sha256") != sha256(args.recommendations)
        or base.get("machine_fingerprint") != calibration.get("machine_fingerprint")
        or base.get("dataset_fingerprint") != calibration.get("dataset_fingerprint")
    ):
        raise ValueError("base recommendations differ from joint calibration")
    corrections = {
        (str(row["benchmark"]), str(row["stage"])): row
